...
ural = WorkCar(80, 'Коричневый', 'Урал', True)
dodge = PoliceCar(130, 'Черный',  'Додж', True)
print(ural.turn_left())
print(f'{ferrari.name} - полицеская машина? {ferrari.is_police}')
print(f'{ural.name} - полицеская машина? {ural.is_police}')
print(f'{lada.go()}, {ferrari.stop()}')
print(f'{ural.go()} со скоростью {ural.show_speed()}')
print(f'{ural.name} имеет {ural.color} цвет.')
